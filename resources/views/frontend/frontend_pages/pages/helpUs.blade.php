...
@else
@section('mytitle','المساعدة للتطوير')
...
<main class="main">
    ...
    <div class="container">
    <div class="page-header" style="background:url('/storage/photos/1/Artboard 1 copy-100.jpg');background-size:cover">
            <h1 class="page-title mb-0 text-white">ساعدنا لتحديث الموقع</h1>
        </div>
    </div>
    <!-- End of Page Header -->
                                    ...
                                    <ul id="chat">
                                        ...
                                        <li class="you">
                                            <div class="entete">
                                                <span class="status green"></span>
                                                <h2>{{ $user->full_name }}</h2>
                                                <h3>{{ $created_at[0] }},في {{ $created_at[1] }}</h3>
                                            </div>
                                            <div class="triangle"></div>
                                            <div class="row">
                                                <div class="col-9">
                                                ...
                                                @if ($user->id == auth()->user()->id)
                                                <div class="col-3">
                                                    <a href="{{ route('edit_post',$post->id) }}"><i class="fa fa-check-circle" style="color: #58b666"></i> تعديل المقال</a><br>
                                                    <a href="{{ route('delete_post',$post->id) }}"><i class="fa fa-trash-alt" style="color: #c02409"></i> حذف المقال</a>
                                                </div>
                                                @endif
                                            </div>
                                            <div style="display:flex">
                                        ...
                                        @endforeach
                                        @else
                                        <strong>كن أول مستخدم يعطينا المساعدة على التحسين.</strong>
                                        @endif
                                    </ul>
                                    <footer>
                                        <form action="{{ route('helpusform') }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
                                            <textarea name="message" placeholder="أكتب هنا ..........."></textarea>
                                            <button class="btn btn-primary" type="submit">أرسل الان</a>
                                        </form>
                                    </footer>
                                </div>
                            </div>
                            @else
                            <strong style="color: red;"> الرجاء تسجيل الدخول أولا لتقديم ومراجعة التعليقات</strong><a href="{{ route('loginForm') }}"> أضغط هنا</a> 
                            @endauth
                        </div>
                    </div>
                </div>
